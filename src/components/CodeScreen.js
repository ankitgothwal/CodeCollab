...
function CodeScreen() {
  ...
  const handleLanguage = (lang) => {
    setLanguage(lang);
    socketRef.current.emit("LANGUAGE_CHANGE", { lang, roomId });
    socketRef.current.emit(USER.MESSAGE, {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      socketRef.current.on("LANGUAGE_CHANGE", ({ lang }) => {
      ...
      socketRef.current.on(USER.JOINED, ({ clients, username, socketId }) => {
        if (username !== userName) {
          ...
          // ⬇️ send full state to new client
          socketRef.current.emit(CODE.SYNC, {
            code,
            question,
            socketId,
      ...
      socketRef.current.on(CODE.SYNC, ({ code, question, language }) => {
        if (code) setCode(code);
